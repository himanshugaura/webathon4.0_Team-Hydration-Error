...
function RegisterContent() {
  ...
  const eventParam = searchParams.get("event") || "coderush";

  const [selectedSlug, setSelectedSlug] = useState(eventParam);

  const event = data.events.find(
    (e) => e.slug === selectedSlug || e.id === selectedSlug
  ) || data.events[0];

  useEffect(() => {
    if (eventParam) {
      setSelectedSlug(eventParam);
    }
  }, [eventParam]);
  ...
  }, [selectedSlug, event.teamSize.min]);
            ...
            <div className="rounded-3xl backdrop-blur-xl bg-white/70 dark:bg-zinc-900/70 border border-zinc-300/70 dark:border-zinc-800/80 p-6 sm:p-10 shadow-sm">
              ...
              <div className="mb-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
                  ...
                  {/* Event selector if arriving on general register page */}
                  <select
                    value={selectedSlug}
                    onChange={(e) => setSelectedSlug(e.target.value)}
                    className="px-3 py-1.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 text-xs font-bold text-zinc-800 dark:text-zinc-200 outline-none cursor-pointer"
                  >
                    {data.events.map((ev) => (
...
export default function GeneralRegistrationPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-sm font-semibold">Loading registration...</div>}>
      <RegisterContent />
    </Suspense>
  );
}
